with prod as (
    select
    ct.category_id, sp.company_name suppliers, pd.product_name, pd.unit_price, pd.product_id
    ...
    select 
    ord.order_date, ord.order_id, cs.company_name customer, em.name employee, em.age, em.lengtofservice
    ...
    select
    od.*, ord.order_date, ord.customer, ord.employee, ord.age, ord.lengtofservice
    from orddetai od
